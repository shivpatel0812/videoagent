...
class VersatileVideoAgent {
  ...
  async processVideo(videoPath, userPrompt) {
    console.log(`🎯 Processing video for: "${userPrompt}"`);
    ...
    for (let i = 0; i < sortedFrames.length; i++) {
      ...
      try {
        ...
        console.log(`Frame ${i + 1}/${sortedFrames.length}: ${objects.length} objects, ${movements.length} movements`);
    ...
    console.log('📚 Indexing data in RAG system...');
    this.ragSystem.indexVideoData(this.extractor.extractedData);
    
    await fs.rm(outputDir, { recursive: true, force: true });
    
    return this.extractor.extractedData;
  }
...
Please provide a detailed answer with specific numbers and insights from the data.`
    ...
    return {
      ...
      rawData: relevantDocs,
      dataExtracted: this.extractor.extractedData
    };
  }
}

async function runVersatileAgent() {
  const videoPath = '/Users/shivpatel/Downloads/18361966-uhd_3840_2160_60fps.mp4';
  const userPrompt = 'Count the unique people in this video';
  
  const agent = new VersatileVideoAgent();
  
  const extractedData = await agent.processVideo(videoPath, userPrompt);
  
  console.log('\n📊 DATA EXTRACTION COMPLETE:');
  console.log(`Objects detected: ${extractedData.objects.length}`);
  console.log(`Movements tracked: ${extractedData.movements.length}`);
  console.log(`Zone interactions: ${extractedData.zones.length}`);
  console.log(`Attribute data: ${extractedData.attributes.length}`);
  
  const result = await agent.queryVideo(userPrompt);
  console.log('\n🤖 AGENT RESPONSE:');
  console.log(result.answer);
  
  console.log('\n🔄 Try follow-up questions:');
  const followUps = [
    'How many people were likely male versus female?',
    'What movements did people make through the door area?',
    'When was the peak activity in the video?'
  ];
  
  for (const followUp of followUps) {
    const followUpResult = await agent.queryVideo(followUp);
    console.log(`\n❓ ${followUp}`);
    console.log(`💬 ${followUpResult.answer}`);
  }
}

runVersatileAgent().catch(console.error);