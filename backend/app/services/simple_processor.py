...
class SimpleVideoProcessor:
    ...
    async def initialize(self):
        print("Simple video processor initialized")
    ...
    def detect_objects_simple(self, image: np.ndarray, frame_number: int) -> List[Dict]:
        if image is None:
            return []
        ...
        for i, contour in enumerate(contours[:15]):
            ...
            if area > 2000:
                confidence = min(0.5 + (area / 15000), 0.95)
                aspect_ratio = w / h if h > 0 else 1.0
                
                if area > 8000 and 0.3 < aspect_ratio < 0.8:
                    object_class = "person"
                    person_count += 1
                elif area > 15000 and aspect_ratio > 1.2:
                    object_class = "car"
                    car_count += 1
                else:
                    object_class = "object"
        ...
        if person_count == 0 and frame_number % 3 == 0:
            mock_detections.append({
                "class": "person",
                "score": 0.75,
                "bbox": [float(width * 0.3), float(height * 0.2), float(width * 0.2), float(height * 0.6)]
            })
    ...
    async def detect_objects_in_frames(self, frame_paths: List[str], job_id: str) -> List[ObjectDetection]:
        ...
        for i, frame_path in enumerate(frame_paths):
            ...
            predictions = self.detect_objects_simple(image, i + 1)
